        ...
        for line in lines:
            ...
            if class_name in specific_class_idxs:
                ...
                for i in range(sample_num_per_sample):
                    ...
                    bbox1 = [new_bbox_left, new_bbox_top, new_bbox_left + bbox_width - 1,
                             new_bbox_top + bbox_height - 1]
                    ious = [bbox_iou(bbox1, bbox) for bbox in bboxes]
                    print(max(ious))
                    if max(ious) <= threshold:
                        # write into txt file
                        fout.write(str(bbox1[0]) + ',' + str(bbox1[1]) + ',' + str(bbox_width) + ',' + str(
                            bbox_height) + ',' + '0,' + str(class_name) + ',0,0' + '\n')
                        # update bboxes list
                        bboxes.append(bbox1)
                        # update image
